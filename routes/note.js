...
router.post('/add', async function (req, res) {
    ...
    try {
        ...
        const { titulo, planta, habitacion, cama, fechaActualizacion, observaciones1, observaciones2, emailCreado, ct_frecuenciaCardiaca, ct_frecuenciaRespiratoria, ct_temperatura, ct_presionArterial } = req.body;

        // Realizamos la consulta
        const sqlQuery = 'INSERT INTO tfg.Nota (titulo, planta, habitacion, cama, fechaActualizacion, observaciones1, observaciones2, emailCreado, ct_frecuenciaCardiaca, ct_frecuenciaRespiratoria, ct_temperatura, ct_presionArterial) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)';
        pool.query(sqlQuery, [titulo, planta, habitacion, cama, fechaActualizacion, observaciones1, observaciones2, emailCreado, ct_frecuenciaCardiaca, ct_frecuenciaRespiratoria, ct_temperatura, ct_presionArterial], function (err, result) {
            if (err) {
    ...
    try {
        ...
        pool.query(sqlQuery, [planta, habitacion, cama, fechaActualizacion, emailCreado], function (err, result) {
            ...
            if(result.affectedRows > 0){
                ...
                res.status(404).send(`No hay ningúna nota con esos datos`)
            }
